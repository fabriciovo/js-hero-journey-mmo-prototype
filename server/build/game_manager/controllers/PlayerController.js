  ...
  (0, _createClass2["default"])(PlayerController, [{
            ...
            while (1) {
              switch (_context2.prev = _context2.next) {
                ...
                case 6:
                  ...
                  socket.emit("currentPlayers", _this2.players, instanceId); // inform the other players of the new player that joined
    ...
    value: function _eventAttackedPlayer(socket) {
      ...
      return socket.on("attackedPlayer", function (attackedPlayerId) {
        if (_this10.players[attackedPlayerId]) {
          ...
          var playerAttackValue = _this10.players[socket.id].attack; // subtract health from attacked player

          _this10.players[attackedPlayerId].playerAttacked(playerAttackValue); // check attacked players health, if dead send gold to other player


          if (_this10.players[attackedPlayerId].health <= 0) {
            // get the amount of gold, and update player object
            _this10.players[socket.id].updateGold(gold); // respawn attacked player


            _this10.players[attackedPlayerId].respawn(_this10.players);

            _this10.io.emit("respawnPlayer", _this10.players[attackedPlayerId]); // send update gold message to player


            socket.emit("updateScore", _this10.players[socket.id].gold); // reset the attacked players gold

            _this10.players[attackedPlayerId].updateGold(-gold);

            _this10.io.to("".concat(attackedPlayerId)).emit("updateScore", _this10.players[attackedPlayerId].gold); // add bonus health to the player
    ...
    value: function _eventPlayerHit(socket) {
      ...
      return socket.on("playerHit", function (playerId, monsterAttack) {
        ...
        _this11.players[playerId].playerAttacked(monsterAttack); // check attacked players health, if dead send gold to other player


        if (_this11.players[playerId].health <= 0) {
          // get the amount of gold, and update player object
          _this11.players[playerId].updateGold(gold); // respawn attacked player


          _this11.players[playerId].respawn(_this11.players);

          _this11.io.emit("respawnPlayer", _this11.players[playerId]); // send update gold message to player


          socket.emit("updateScore", _this11.players[socket.id].gold); // reset the attacked players gold

          _this11.players[playerId].updateGold(-gold);

          _this11.io.to("".concat(playerId)).emit("updateScore", _this11.players[playerId].gold); // add bonus health to the player
    ...
    value: function _eventSendBuyItemMessage(socket) {
      ...
      return socket.on("sendBuyItemMessage", function (item) {
        ...
        socket.broadcast.emit("updatePlayersScore", socket.id, _this12.players[socket.id].gold); // check the player's health, if below 0 have the player respawn
        ...
        if (_this12.players[playerId].health <= 0) {
          ...
          socket.emit("updateScore", _this12.players[playerId].gold); // respawn the player
    ...
    value: function _eventDisconnect(socket) {
      ...
      return socket.on("disconnect", function () {
        ...
        delete _this14.players[socket.id]; // emit a message to all players to remove this player
